...
impl AsDb {
    ...
    pub fn from_entries(entries: Vec<AsEntry>) -> Self {
        let inner = entries
            .into_iter()
            .map(|entry| (entry.asn, entry.name))
            .collect();
        Self { inner }
    }
...
mod tests {
    ...
    fn test_serialization_roundtrip() {
        let entries = vec![AsEntry {
            asn: 64500,
            name: "TestNet".into(),
        }];

        let mut buf = Vec::new();
        bincode::serde::encode_into_std_write(&entries, &mut buf, bincode::config::standard())
            .unwrap();
